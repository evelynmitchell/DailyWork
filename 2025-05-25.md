...
https://www.printmag.com/designer-profiles/miniature-knitter-julie-steiner/  #knitting

https://huggingface.co/lightonai/Reason-ModernColBERT #embedding index and retrieve documents using ColBERT models "Reason-ModernColBERT is a late interaction model trained on the [reasonir-hq](https://huggingface.co/datasets/reasonir/reasonir-data) dataset. It achieves extremely competitive performance on the [BRIGHT benchmark](https://huggingface.co/datasets/xlangai/BRIGHT) aimed at evaluating reasoning-intensive retrieval performance, outperforming all existing models up to 7B (more than 45 times its size) and even surprisingly improving performance of [ReasonIR-8B](https://huggingface.co/reasonir/ReasonIR-8B) (a 8B model trained on the same data) by more than 2.5 NDCG@10 on average on Stack Exchange splits. We attribute such strong results to late-interaction, see [evaluation section](https://huggingface.co/lightonai/Reason-ModernColBERT#evaluation)." https://github.com/lightonai/pylate #finetuning "PyLate is a library built on top of Sentence Transformers, designed to simplify and optimize fine-tuning, inference, and retrieval with state-of-the-art ColBERT models. It enables easy fine-tuning on both single and multiple GPUs, providing flexibility for various hardware setups. PyLate also streamlines document retrieval and allows you to load a wide range of models, enabling you to construct ColBERT models from most pre-trained language models." https://lightonai.github.io/pylate/
# agent prompts
